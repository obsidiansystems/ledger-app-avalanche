    ...
    switch (state->state) {
      ...
      case 1: {
        switch (state->type) {
          ...
          case LEGACY: {
            PRINTF("---BEFORE OUTER parse_legacy_rlp_txn\n");
            sub_rv = parse_legacy_rlp_txn(&state->txn_state, meta);
            PRINTF("---BEFORE OUTER RET_IF_NOT_DONE\n");
            PRINTF("== LEGACY case meta->input.consumed: %d \n", meta->input.consumed);
            RET_IF_NOT_DONE;
            PRINTF("---AFTER OUTER RET_IF_NOT_DONE\n");
            break;
          }
...
enum parse_rv parse_legacy_rlp_txn(struct EVM_RLP_txn_state *const state, evm_parser_meta_state_t *const meta) {
    enum parse_rv sub_rv = PARSE_RV_INVALID;
    PRINTF("=================== meta->input.consumed: %d, state: %d \
           \n", meta->input.consumed, state->state);
    switch(state->state) {
      case 0: {
          ...
          PRINTF("consumed in parse_legacy_rlp_txn 364 ----\n");
            ...
            case 2:
              ...
              case TXN_DATA_CONTRACT_CALL_UNKNOWN_DEST: {
                ...
                if(item_state->do_init) {
                  PRINTF("CALL DATA INIT\n");
                  init_abi_call_data(abi_state, item_state->length);
                  item_state->do_init = false;
                }

                PRINTF("TEMP 3 PER ITEM PROMPT %d\n", state->per_item_prompt);
              ...
              case TXN_DATA_PLAIN_TRANSFER:
                PRINTF("ITEM 4 PER ITEM PROMPT %d\n", state->per_item_prompt);
              ...
              if (sub_rv == PARSE_RV_PROMPT) {
                PRINTF("THERE ALSO NEED PROMPT %d %d\n", state->per_item_prompt, sub_rv);
                ...
                PRINTF("THERE ALSO NEED PROMPT %d %d\n", state->per_item_prompt, state->item_rv);
 ...
 rebranch:
    ...
    switch(state->state) {
      ...
      case 3: {
        ...
        if(max_bytes_to_buffer) {
          PRINTF("consumed in parse_rv 1074 ----\n");
...
enum parse_rv parse_abi_call_data(struct EVM_ABI_state *const state,
  ...
  enum parse_rv sub_rv;
rebranch:
  PRINTF("parse abi call data state=%d\n", state->state);
  switch(state->state) {
  ...
  case ABISTATE_ARGUMENTS: {
    PRINTF("CALL DATA ARGUMENT COUNT %d\n", meta->known_endpoint->parameters_count);
    while (state->argument_index < meta->known_endpoint->parameters_count) {
      PRINTF("CALL DATA ARGUMENT %d\n", state->argument_index);
      ...
      initFixed(fs(&state->argument_state), sizeof(state->argument_state));
      PRINTF("CALL DATA argument name: %s\n", argument_name);
  ...
  PRINTF("Returning from parse abi call data state=%d ret=%d\n", state->state, sub_rv);
  return sub_rv;
}
